...
class Venue(db.Model):
    ...
    # secondary links this to the associative (m tow n) table name
    ...
    shows = db.relationship('Show', backref='venue', lazy='joined')    # Can reference show.venue (as well as venue.shows)
...
class Artist(db.Model):
    ...
    # In the parent is where we put the db.relationship in SQLAlchemy
    shows = db.relationship('Show', backref='artist', lazy='joined')    # Can reference show.artist (as well as artist.shows)
...
class Show(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    start_time = db.Column(db.DateTime, nullable=False, default=datetime.datetime)    # Start time required field
