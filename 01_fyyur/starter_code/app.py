...
from forms import *
import sys
from model import db, Show, Venue, Artist, Genre, venue_genre_table, artist_genre_table
#----------------------------------------------------------------------------#
# App Config.
#----------------------------------------------------------------------------#
...
def venues():
  ...
  data = []
  for ven in  data_venue:
    Check_city = True
    for city in data:
      if city['city'] == ven.city:
        Check_city = False
    if Check_city:
      venue = []
      for i in data_venue:
...
def create_venue_submission():
  ...
  try:
    ...
    venue = Venue(name = name, city = city, state = state, address = address,phone = phone,
      ...
      seeking_talent = bool(looking_for_talent), seeking_description = seeking_description)
    for genre in genres:
...
def show_artist(artist_id):
  ...
  # }
  # data = list(filter(lambda d: d['id'] == artist_id, data))
...
def create_artist_submission():
  ...
  try:
    ...
    artist = Artist(name = name, city = city, state = state, phone = phone, 
      facebook_link = facebook_link, image_link = image_link, website_link = website_link, 
      seeking_venue = bool(looking_for_venues), seeking_description = seeking_description)
    for genre in genres:
      # fetch_genre = session.query(Genre).filter_by(name=genre).one_or_none()  # Throws an exception if more than one returned, returns None if none
      fetch_genre = Genre.query.filter_by(name=genre).one_or_none()  # Throws an exception if more than one returned, returns None if none
      if fetch_genre:
...
def create_show_submission():
  ...
  if error:
    ...
    flash('Wrong Artist or Venue ID')
  # TODO: on unsuccessful db insert, flash an error instead.
